...
static void update_objects(const std::vector<Object>& objects, string ip, int port, string request_name)
{
    ...
    if (objects.size() > 0) {
        char text[256*int(objects.size())];
        ...
        //fprintf(stderr, "%s\n", out);
        string ret = out;
        post_request(ip, port, request_name, ret);
    } else {
        string ret = "{\"output\": []}";
        post_request(ip, port, request_name, ret);
    }
...
int main(int argc, char** argv) {
    ...
    string ip="127.0.0.1";
    int port=80;
    string request_name = "detect_result";
    ...
    std::string help_string = "\nFormat: -[Options] [Params]\n\n" \
                                ...
                                "-name      POST request name(default detect_result)\n" \
                                "-h         show this help\n\n";
    ...
    while(i<argc) {
        ...
        else if (std::string(argv[i]) == "-name") {const std::string rnt {argv[i+1]};request_name = rnt;}
        i+=2;
    }
    ...
    if(sourcetype==0){
        ...
        update_objects(objects, ip, port, request_name);
        //cv::waitKey(0);
    }
